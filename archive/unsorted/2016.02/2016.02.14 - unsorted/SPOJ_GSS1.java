...
public class SPOJ_GSS1 {
    ...
    public void solve(int testNumber, InputReader input, PrintWriter out) {
        ...
        for(int i = 0; i < m; ++i)
            queries[i] = new Query(in.i() - 1, in.i() - 1);
    ...
    class SegTree {
        ...
        Node merge(Node lchild, Node rchild) {
            ...
            rs.max_l = Math.max(lchild.max_l, Math.max(lchild.sum, lchild.sum + rchild.max_l));
            rs.max_r = Util.max(rchild.max_r, rchild.sum, lchild.max_r + rchild.sum);
